...
export class Bot {
  constructor(id, name, lanePositions, startLane, speedMultiplier, colors) {
    ...
    this._shakeDurationMs = 350;
    this._hitEffectPending = false;
  }
  ...
  applyHitSlowdown(durationMs, factor) {
    ...
    // Trigger a one-shot boat-shake + impact-particle cue. Game.js consumes
    // _hitEffectPending once per hit (via consumeHitEffect) to spawn
    // particles at the bot's current screen position, since Bot itself has
    // no renderer access.
    this._shakeMsRemaining = this._shakeDurationMs;
    this._hitEffectPending = true;
  }
  ...
  /** Small decaying horizontal jitter while _shakeMsRemaining > 0. */
  get shakeOffsetX() {
    if (this._shakeMsRemaining <= 0) return 0;
    const t = this._shakeMsRemaining / this._shakeDurationMs; // 1 -> 0
    const amplitude = 6 * t;
    return Math.sin(this._shakeMsRemaining * 0.09) * amplitude;
  }

  /** Returns true exactly once per hit, then resets — for one-shot particle spawns. */
  consumeHitEffect() {
    if (!this._hitEffectPending) return false;
    this._hitEffectPending = false;
    return true;
  }
