...
export class UIManager {
  ...
  _renderRaceResults(context) {
    ...
    this.el.raceResultsHeading.textContent = `${medal} ${rank}${suffix} PLACE${rank === 1 ? '!' : ''}`;
    ...
    this.el.raceResultsCoins.textContent = `+${coinsAwarded} COINS 🪙`;

    // Small satisfying pop-in for the coin reward — reuses the same entrance
    // animation as shop purchases (see .purchase-flash / purchasePop in ui.css).
    this.el.raceResultsCoins.classList.remove('coin-reward-pop');
